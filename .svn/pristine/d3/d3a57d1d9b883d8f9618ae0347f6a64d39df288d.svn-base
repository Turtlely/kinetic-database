from kdb.aselite import Atoms
from kdb.aselite import FixAtoms
from kdb.aselite import write_vasp
from kdb.aselite import elements_groups
import numpy as np
import pymysql
...
class RemoteDB():
    ...
    def add_process(self, o_reactant, o_saddle, o_product, o_mode,
                    reactant, saddle, product, mode, mobile_set,email,passwd,b_f,b_r ):
        ...
        if b_f is not 0 and type(b_f) != 'NoneType': # Need this check for the website insert untill we allow users to submit barriers
            barrier_forward = float(b_f)
            barrier_reverse = float(b_r)
        else:
            barrier_forward = 0
            barrier_reverse = 0
        ...
        conn.execute('''INSERT INTO Process 
                     ...
                     (o_pro_pk, name, or_pk, os_pk, op_pk,elemental_code,elemental_type_code,barrier_forward,barrier_reverse,user_id))
        conn.execute('COMMIT')
        conn.close()
        ...
        conn.execute('''INSERT INTO Process 
                        VALUES ('%d','%s','%d','%d','%d', '%d','%d','%d','%f','%f')''' %
                     (pro_pk, name, o_pro_pk, r_pk, s_pk, p_pk, elemental_code,elemental_type_code,barrier_forward,barrier_reverse))
    ...
    def get_process(self, name):
        ...
        #retrieve all kdb entries
        conn.execute('''SELECT name FROM Process''')
        prime_tuple = conn.fetchall()
        ...
        if config.USE_GROUP_SIMILARITY: #if flag true in config.py, get process list based on elemental_type_code
            ...
            print (query_code," Similarity query_code (7 for G13)")
            conn.execute('''SELECT * FROM Process WHERE  %s%%elemental_type_code  = 0''',(query_code,)) #select all processes w$
            process_list = conn.fetchall()
            #print process_list,"similar_atom process list"
        else:
            query_code = Kdb().name_to_code(name)
            conn.execute('''SELECT * FROM Process WHERE %s%%elemental_code = 0''',(query_code,)) #select process if query_code % elemental_c
            process_list = conn.fetchall()
        print (process_list)
        for process in process_list:
            ...
            pro_dict['barrier'] = [process[8],process[9]]
