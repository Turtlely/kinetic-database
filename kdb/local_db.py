...
class LocalDB():
    def __init__(self, db_name, nf=None, dc=None, mac=None):
        ...
        if self.check_params():
            ...
            answer = input("Do you want to update database entries to reflect the new parameters? (yes, no)")
    ...
    def create_tables(self):
        ...
        conn.execute('''CREATE TABLE Process(pro_id INT PRIMARY KEY UNIQUE NOT NULL,
                                             ...
                                             product_id           INT,
                                             elemental_code       INT,
                                             elemental_type_code  INT,
                                             barrier_forward      REAL,
                                             barrier_reverse      REAL,
        ...
        default_values = [('nf', config.NEIGHBOR_FUDGE), ('dc', config.DISTANCE_CUTOFF), ('mac', config.MOBILE_ATOM_CUTOFF)]
    ...
    def add_process(self, o_reactant, o_saddle, o_product, o_mode, 
                    reactant, saddle, product, mode, mobile_set, b_f, b_r,
                    clump_order_r, clump_order_s, clump_order_p):
        ...
        if reactant:
            r_pk  = self.add_atoms(reactant, mode, clump_order_r, conn)
            s_pk  = self.add_atoms(saddle, mode, clump_order_s, conn)
            p_pk  = self.add_atoms(product, mode, clump_order_p, conn)
            is_used = 1
        else:
            r_pk = None
            s_pk = None
            p_pk = None
            is_used = 0
        # make the process name using mobile+neighbor atoms from saddle rather than original reactant
        symbols = saddle.get_chemical_symbols()
        #symbols = o_reactant.get_chemical_symbols()
        name = self.get_name(symbols)
        elemental_code = Kdb().name_to_code(name)
        elemental_type_code = Kdb().name_to_type_code(name)
        ...
        conn.execute('''INSERT INTO Process VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)''', 
                     (pro_pk, name, is_used, or_pk, os_pk, op_pk, r_pk, s_pk, p_pk, elemental_code,elemental_type_code,barrier_forward,barrier_reverse)
                    )
    ...
    def add_atoms(self, atoms, mode, clump_order, conn):
    ...
    def get_atoms(self, atoms_id):
        ...
        atoms = conn.execute('SELECT * FROM Atom WHERE atoms_id = ?', (str(atoms_id),) ).fetchall()
        conn.close()
        symbols = []
        positions = []
        fixed = []
        ...
        cell = [[1024,0,0], [0,1024,0], [0,0,1024]]
    ...
    def get_process(self, name):
        ...
        # query db for all processes with same name as given in params
        conn = self.connect_db()
        prime_tuple = conn.execute('''SELECT name FROM Process''').fetchall()
        kdb_list = list(map(' '.join,prime_tuple))
        if config.USE_GROUP_SIMILARITY:  # if flag true in config.py, get process list based on elemental_type_code
            query_code = Kdb().name_to_type_code(name)
            process_list = conn.execute('''SELECT * FROM Process WHERE  ?%elemental_type_code  = 0''',
                           (query_code,)).fetchall()  # select process if query_code is divisible by elemental_type_code
        else:
            query_code = Kdb().name_to_code(name)
            process_list = conn.execute('''SELECT * FROM Process WHERE  ?%elemental_code  = 0''',
                           (query_code,)).fetchall()  # select process if query_code is divisible by elemental_code
        conn.close()

        if config.ENERGY_CEILING != 0.0:
            temp_list = []
            for i in range(len(process_list)):
                if ( min(process_list[i][10], process_list[i][11]) ) < config.ENERGY_CEILING and \
                   ( min(process_list[i][10], process_list[i][11]) ) != 0.0:  # only those below the ceiling AND barrier is not unknown (default is 0.0)
                    temp_list.append(process_list[i])
            process_list = temp_list
        if config.ENERGY_FLOOR != 0.0:
            temp_list = []
            for i in range(len(process_list)):
                if ( max(process_list[i][10], process_list[i][11]) ) > config.ENERGY_FLOOR and \
                   ( max(process_list[i][10], process_list[i][11]) ) != 0.0: 
                    temp_list.append(process_list[i])
            process_list = temp_list
        ...
        with mp.Pool(cpuCount) as pool:
            return_list = []
            for result in pool.map(self.make_process_list, process_list):
                return_list += result
        return return_list

    def make_process_list(self, process):
        return_list = []
        ...
        pro_dict['minimum'], pro_dict['clump_order'] = self.get_atoms(process[6])
        pro_dict['saddle'], _  = self.get_atoms(process[7])
        pro_dict['product'], temp = self.get_atoms(process[8])
        pro_dict['mode']    = self.get_mode(process[6])
        pro_dict['id']      = process[0]
        pro_dict['barrier'] = [process[10],process[11]]
        return_list.append(pro_dict)
        #pro_dict with mirror set to true
        #NK we do not need a separate dictionary for mirror
        # swap min and product and re add dictionary
        swap_dict = copy.deepcopy(pro_dict)  # this line and next 3 lines enable product to also be treated as a reactant
        swap_dict['minimum'], swap_dict['product'] = swap_dict['product'], swap_dict['minimum']
        swap_dict['clump_order'] = temp  # clump order of the product
        swap_dict['id'] *= -1  # change id to distinguish forward vs backwards process
    ...
    def update_params(self):
        ...
        for process in process_list:
            o_reactant, _    = self.get_atoms(process[3])
            o_saddle, _      = self.get_atoms(process[4])
            o_product, _     = self.get_atoms(process[5])
            o_mode        = self.get_mode(process[3])
            kdbupdate = LocalUpdate()
    ...
    #We had a bug here, if symbols = ['Cu' 'C', 'H'] -> name comes out CuH
    #Cannot believe we had this bug, updated the bug
